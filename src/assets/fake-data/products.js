const product_01_image_01 = ('../model/sneaker_1/scene.gltf')
const product_01_image_02 = ('../model/sneaker_1/scene.usdz')

const product_02_image_01 = ('../model/sneaker_2/scene.gltf')
const product_02_image_02 = ('../model/sneaker_2/scene.usdz')

const product_03_image_01 = ('../model/sneaker_3/scene.gltf')
const product_03_image_02 = ('../model/sneaker_3/scene.usdz')

const product_04_image_01 = ('../model/sneaker_4/scene.gltf')
const product_04_image_02 = ('../model/sneaker_4/scene.usdz')

const product_05_image_01 = ('../model/sneaker_5/scene.gltf')
const product_05_image_02 = ('../model/sneaker_5/scene.usdz')

const product_06_image_01 = ('../model/sneaker_6/scene.gltf')
const product_06_image_02 = ('../model/sneaker_6/scene.usdz')

const product_07_image_01 = ('../model/sneaker_7/scene.gltf')
const product_07_image_02 = ('../model/sneaker_7/scene.usdz')

const product_08_image_01 = ('../model/sneaker_8/scene.gltf')
const product_08_image_02 = ('../model/sneaker_8/scene.usdz')

const product_09_image_01 = ('../model/sneaker_9/scene.gltf')
const product_09_image_02 = ('../model/sneaker_9/scene.usdz')

const product_10_image_01 = ('../model/sneaker_10/scene.gltf')
const product_10_image_02 = ('../model/sneaker_10/scene.usdz')

const product_11_image_01 = ('../model/sneaker_11/scene.gltf')
const product_11_image_02 = ('../model/sneaker_11/scene.usdz')

const product_12_image_01 = ('../model/sneaker_12/scene.gltf')
const product_12_image_02 = ('../model/sneaker_12/scene.usdz')

const product_13_image_01 = ('../model/sneaker_13/scene.gltf')
const product_13_image_02 = ('../model/sneaker_13/scene.usdz')

const product_14_image_01 = ('../model/sneaker_14/scene.gltf')
const product_14_image_02 = ('../model/sneaker_14/scene.usdz')

const product_15_image_01 = ('../model/sneaker_15/scene.gltf')
const product_15_image_02 = ('../model/sneaker_15/scene.usdz')

const product_16_image_01 = ('../model/sneaker_16/scene.gltf')
const product_16_image_02 = ('../model/sneaker_16/scene.usdz')

const product_17_image_01 = ('../model/sneaker_17/scene.gltf')
const product_17_image_02 = ('../model/sneaker_17/scene.usdz')

const product_18_image_01 = ('../model/sneaker_18/scene.gltf')
const product_18_image_02 = ('../model/sneaker_18/scene.usdz')

const product_19_image_01 = ('../model/sneaker_19/scene.gltf')
const product_19_image_02 = ('../model/sneaker_19/scene.usdz')

const product_20_image_01 = ('../model/sneaker_20/scene.gltf')
const product_20_image_02 = ('../model/sneaker_20/scene.usdz')

const product_21_image_01 = ('../model/sneaker_21/scene.gltf')
const product_21_image_02 = ('../model/sneaker_21/scene.usdz')

const products = [
    {
        title: "sneaker_1",
        price: '2500',
        image01: product_01_image_01,
        image02: product_01_image_02,
        categorySlug: "air-dior",
        colors: ["ホワイト"],
        slug: "sneaker_1",
        size: ["18.0", "18.5", "19.0", "19.5", "20.0", "20.5", "21.0", "21.5", "22.0"],
        description: "通気性抜群の履き心地。軽い素材で走りやすく、靴底は厚くしっかりカバーされるようになっています。湿気には弱く手入れが必要になります。防水スプレーの購入もお勧めします。ご不明な点はご質問くだざい。よろしくお願いします。",
        contactTitle_1:"通気性はありますか？",
        contactContent_1:"あります。汗による蒸れを防ぎます。",
        contactTitle_2: "重いですか？",
        contactContent_2: "この商品の重さは350gです。350ml入りのペットボトルの重さ程度でとても軽い方です。",
        contactTitle_3: "防水性はありますか？",
        contactContent_3: "防水性はありません。湿気による傷みが発生する可能性があるので、しっかりケアをしましょう。別売りの防水スプレーも一緒に購入をオススメします。",


    },

    {
        title: "sneaker_2",
        price: '2500',
        image01: product_02_image_01,
        image02: product_02_image_02,
        categorySlug: "nike",
        colors: ["ホワイト"],
        slug: "sneaker_2",
        size: ["18.0", "18.5", "19.0", "19.5", "20.0", "20.5", "21.0", "21.5", "22.0"],
        description: "通気性抜群の履き心地。軽い素材で走りやすく、靴底は厚くしっかりカバーされるようになっています。湿気には弱く手入れが必要になります。防水スプレーの購入もお勧めします。ご不明な点はご質問くだざい。よろしくお願いします。",

    },
    {
        title: "sneaker_3",
        price: '2500',
        image01: product_03_image_01,
        image02: product_03_image_02,
        categorySlug: "nike",
        colors: ["ホワイト"],
        slug: "sneaker_3",
        size: ["18.0", "18.5", "19.0", "19.5", "20.0", "20.5", "21.0", "21.5", "22.0"],
        description: "通気性抜群の履き心地。軽い素材で走りやすく、靴底は厚くしっかりカバーされるようになっています。湿気には弱く手入れが必要になります。防水スプレーの購入もお勧めします。ご不明な点はご質問くだざい。よろしくお願いします。",

    },
    {
        title: "sneaker_4",
        price: '2500',
        image01: product_04_image_01,
        image02: product_04_image_02,
        categorySlug: "nike",
        colors: ["ブル"],
        slug: "sneaker_4",
        size: ["18.0", "18.5", "19.0", "19.5", "20.0", "20.5", "21.0", "21.5", "22.0"],
        description: "通気性抜群の履き心地。軽い素材で走りやすく、靴底は厚くしっかりカバーされるようになっています。湿気には弱く手入れが必要になります。防水スプレーの購入もお勧めします。ご不明な点はご質問くだざい。よろしくお願いします。",

    },
    {
        title: "sneaker_5",
        price: '2500',
        image01: product_05_image_01,
        image02: product_05_image_02,
        categorySlug: "nike",
        colors: ["ブル"],
        slug: "sneaker_5",
        size: ["18.0", "18.5", "19.0", "19.5", "20.0", "20.5", "21.0", "21.5", "22.0"],
        description: "通気性抜群の履き心地。軽い素材で走りやすく、靴底は厚くしっかりカバーされるようになっています。湿気には弱く手入れが必要になります。防水スプレーの購入もお勧めします。ご不明な点はご質問くだざい。よろしくお願いします。",

    },
    {
        title: "sneaker_6",
        price: '2500',
        image01: product_06_image_01,
        image02: product_06_image_02,
        categorySlug: "nike",
        colors: ["グレー"],
        slug: "sneaker_6",
        size: ["18.0", "18.5", "19.0", "19.5", "20.0", "20.5", "21.0", "21.5", "22.0"],
        description: "通気性抜群の履き心地。軽い素材で走りやすく、靴底は厚くしっかりカバーされるようになっています。湿気には弱く手入れが必要になります。防水スプレーの購入もお勧めします。ご不明な点はご質問くだざい。よろしくお願いします。",

    },
    {
        title: "sneaker_7",
        price: '2500',
        image01: product_07_image_01,
        image02: product_07_image_02,
        categorySlug: "nike",
        colors: ["white"],
        slug: "sneaker_7",
        size: ["18.0", "18.5", "19.0", "19.5", "20.0", "20.5", "21.0", "21.5", "22.0"],
        description: "通気性抜群の履き心地。軽い素材で走りやすく、靴底は厚くしっかりカバーされるようになっています。湿気には弱く手入れが必要になります。防水スプレーの購入もお勧めします。ご不明な点はご質問くだざい。よろしくお願いします。",

    },
    {
        title: "sneaker_8",
        price: '2500',
        image01: product_08_image_01,
        image02: product_08_image_02,
        categorySlug: "nike",
        colors: ["ブラック"],
        slug: "sneaker_8",
        size: ["18.0", "18.5", "19.0", "19.5", "20.0", "20.5", "21.0", "21.5", "22.0"],
        description: "通気性抜群の履き心地。軽い素材で走りやすく、靴底は厚くしっかりカバーされるようになっています。湿気には弱く手入れが必要になります。防水スプレーの購入もお勧めします。ご不明な点はご質問くだざい。よろしくお願いします。",

    },
    {
        title: "sneaker_9",
        price: '2500',
        image01: product_09_image_01,
        image02: product_09_image_02,
        categorySlug: "nike",
        colors: ["ホワイト"],
        slug: "sneaker_9",
        size: ["18.0", "18.5", "19.0", "19.5", "20.0", "20.5", "21.0", "21.5", "22.0"],
        description: "通気性抜群の履き心地。軽い素材で走りやすく、靴底は厚くしっかりカバーされるようになっています。湿気には弱く手入れが必要になります。防水スプレーの購入もお勧めします。ご不明な点はご質問くだざい。よろしくお願いします。",

    },
    {
        title: "sneaker_10",
        price: '2500',
        image01: product_10_image_01,
        image02: product_10_image_02,
        categorySlug: "nike",
        colors: ["ホワイト"],
        slug: "sneaker_10",
        size: ["18.0", "18.5", "19.0", "19.5", "20.0", "20.5", "21.0", "21.5", "22.0"],
        description: "通気性抜群の履き心地。軽い素材で走りやすく、靴底は厚くしっかりカバーされるようになっています。湿気には弱く手入れが必要になります。防水スプレーの購入もお勧めします。ご不明な点はご質問くだざい。よろしくお願いします。",

    },
    {
        title: "sneaker_11",
        price: '2500',
        image01: product_11_image_01,
        image02: product_11_image_02,
        categorySlug: "nike",
        colors: ["white"],
        slug: "sneaker_11",
        size: ["18.0", "18.5", "19.0", "19.5", "20.0", "20.5", "21.0", "21.5", "22.0"],
        description: "通気性抜群の履き心地。軽い素材で走りやすく、靴底は厚くしっかりカバーされるようになっています。湿気には弱く手入れが必要になります。防水スプレーの購入もお勧めします。ご不明な点はご質問くだざい。よろしくお願いします。",

    },
    {
        title: "sneaker_12",
        price: '2500',
        image01: product_12_image_01,
        image02: product_12_image_02,
        categorySlug: "nike",
        colors: ["ホワイト"],
        slug: "sneaker_12",
        size: ["18.0", "18.5", "19.0", "19.5", "20.0", "20.5", "21.0", "21.5", "22.0"],
        description: "通気性抜群の履き心地。軽い素材で走りやすく、靴底は厚くしっかりカバーされるようになっています。湿気には弱く手入れが必要になります。防水スプレーの購入もお勧めします。ご不明な点はご質問くだざい。よろしくお願いします。",

    },
    {
        title: "sneaker_13",
        price: '2500',
        image01: product_13_image_01,
        image02: product_13_image_02,
        categorySlug: "nike",
        colors: ["white"],
        slug: "sneaker_13",
        size: ["18.0", "18.5", "19.0", "19.5", "20.0", "20.5", "21.0", "21.5", "22.0"],
        description: "通気性抜群の履き心地。軽い素材で走りやすく、靴底は厚くしっかりカバーされるようになっています。湿気には弱く手入れが必要になります。防水スプレーの購入もお勧めします。ご不明な点はご質問くだざい。よろしくお願いします。",

    },
    {
        title: "sneaker_14",
        price: '2500',
        image01: product_14_image_01,
        image02: product_14_image_02,
        categorySlug: "nike",
        colors: ["white"],
        slug: "sneaker_14",
        size: ["18.0", "18.5", "19.0", "19.5", "20.0", "20.5", "21.0", "21.5", "22.0"],
        description: "通気性抜群の履き心地。軽い素材で走りやすく、靴底は厚くしっかりカバーされるようになっています。湿気には弱く手入れが必要になります。防水スプレーの購入もお勧めします。ご不明な点はご質問くだざい。よろしくお願いします。",

    },
    {
        title: "sneaker_15",
        price: '2500',
        image01: product_15_image_01,
        image02: product_15_image_02,
        categorySlug: "nike",
        colors: ["white"],
        slug: "sneaker_15",
        size: ["18.0", "18.5", "19.0", "19.5", "20.0", "20.5", "21.0", "21.5", "22.0"],
        description: "通気性抜群の履き心地。軽い素材で走りやすく、靴底は厚くしっかりカバーされるようになっています。湿気には弱く手入れが必要になります。防水スプレーの購入もお勧めします。ご不明な点はご質問くだざい。よろしくお願いします。",

    },
    {
        title: "sneaker_16",
        price: '2500',
        image01: product_16_image_01,
        image02: product_16_image_02,
        categorySlug: "nike",
        colors: ["white"],
        slug: "sneaker_16",
        size: ["18.0", "18.5", "19.0", "19.5", "20.0", "20.5", "21.0", "21.5", "22.0"],
        description: "通気性抜群の履き心地。軽い素材で走りやすく、靴底は厚くしっかりカバーされるようになっています。湿気には弱く手入れが必要になります。防水スプレーの購入もお勧めします。ご不明な点はご質問くだざい。よろしくお願いします。",

    },
    {
        title: "sneaker_17",
        price: '2500',
        image01: product_17_image_01,
        image02: product_17_image_02,
        categorySlug: "nike",
        colors: ["white"],
        slug: "sneaker_17",
        size: ["18.0", "18.5", "19.0", "19.5", "20.0", "20.5", "21.0", "21.5", "22.0"],
        description: "通気性抜群の履き心地。軽い素材で走りやすく、靴底は厚くしっかりカバーされるようになっています。湿気には弱く手入れが必要になります。防水スプレーの購入もお勧めします。ご不明な点はご質問くだざい。よろしくお願いします。",

    },
    {
        title: "sneaker_18",
        price: '2500',
        image01: product_18_image_01,
        image02: product_18_image_02,
        categorySlug: "nike",
        colors: ["white"],
        slug: "sneaker_18",
        size: ["18.0", "18.5", "19.0", "19.5", "20.0", "20.5", "21.0", "21.5", "22.0"],
        description: "通気性抜群の履き心地。軽い素材で走りやすく、靴底は厚くしっかりカバーされるようになっています。湿気には弱く手入れが必要になります。防水スプレーの購入もお勧めします。ご不明な点はご質問くだざい。よろしくお願いします。",

    },
    {
        title: "sneaker_19",
        price: '2500',
        image01: product_19_image_01,
        image02: product_19_image_02,
        categorySlug: "nike",
        colors: ["ブル"],
        slug: "sneaker_19",
        size: ["18.0", "18.5", "19.0", "19.5", "20.0", "20.5", "21.0", "21.5", "22.0"],
        description: "通気性抜群の履き心地。軽い素材で走りやすく、靴底は厚くしっかりカバーされるようになっています。湿気には弱く手入れが必要になります。防水スプレーの購入もお勧めします。ご不明な点はご質問くだざい。よろしくお願いします。",

    },
    {
        title: "sneaker_20",
        price: '2500',
        image01: product_20_image_01,
        image02: product_20_image_02,
        categorySlug: "nike",
        colors: ["white"],
        slug: "sneaker_20",
        size: ["18.0", "18.5", "19.0", "19.5", "20.0", "20.5", "21.0", "21.5", "22.0"],
        description: "通気性抜群の履き心地。軽い素材で走りやすく、靴底は厚くしっかりカバーされるようになっています。湿気には弱く手入れが必要になります。防水スプレーの購入もお勧めします。ご不明な点はご質問くだざい。よろしくお願いします。",

    },
    {
        title: "sneaker_21",
        price: '2500',
        image01: product_21_image_01,
        image02: product_21_image_02,
        categorySlug: "nike",
        colors: ["white"],
        slug: "sneaker_21",
        size: ["18.0", "18.5", "19.0", "19.5", "20.0", "20.5", "21.0", "21.5", "22.0"],
        description: "通気性抜群の履き心地。軽い素材で走りやすく、靴底は厚くしっかりカバーされるようになっています。湿気には弱く手入れが必要になります。防水スプレーの購入もお勧めします。ご不明な点はご質問くだざい。よろしくお願いします。",

    },


    // 18 products
]

const getAllProducts = () => products

const getProducts = (count) => {
    const max = products.length - count
    const min = 0
    const start = Math.floor(Math.random() * (max - min) + min)
    return products.slice(start, start + count)
}

const getProductBySlug = (slug) => products.find(e => e.slug === slug)

const getCartItemsInfo = (cartItems) => {
    let res = []
    if (cartItems.length > 0) {
        cartItems.forEach(e => {
            let product = getProductBySlug(e.slug)
            res.push({
                ...e,
                product: product
            })
        })
    }
    // console.log(res)
    // console.log('sorted')
    // console.log(res.sort((a, b) => a.slug > b.slug ? 1 : (a.slug < b.slug ? -1 : 0)))
    return res.sort((a, b) => a.id > b.id ? 1 : (a.id < b.id ? -1 : 0))
}

const productData = {
    getAllProducts,
    getProducts,
    getProductBySlug,
    getCartItemsInfo
}

export default productData